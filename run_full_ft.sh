#!/bin/bash
# run_full_ft.sh — Full FT chain only, 4-domain cross-domain eval matrix
# Includes VRAM cleanup on start and disk check before every checkpoint save.

chmod +x "$0"
NTFY_TOPIC="${1:-proteus-aman-2026}"
LOG="results/run_full_ft.log"
STEPS=2000
N_EVAL=200
MIN_DISK_GB=62  # abort if less than this free before any train run (~57GB per checkpoint + margin)
...
STEP_FILE="results/.current_step_full_ft"
STATUS_FILE="results/.live_status_full_ft.json"
...
check_disk() {
    local avail_kb
    avail_kb=$(df /workspace --output=avail 2>/dev/null | tail -1 || df / --output=avail | tail -1)
    local avail_gb=$(( avail_kb / 1024 / 1024 ))
    log "Disk check: ${avail_gb}GB free (need ${MIN_DISK_GB}GB)"
    if (( avail_gb < MIN_DISK_GB )); then
        log "ABORT: only ${avail_gb}GB free, need ${MIN_DISK_GB}GB. Aborting before checkpoint write."
        notify "Full FT ABORTED — disk full" \
"Only ${avail_gb}GB free before checkpoint save.
Step: $(get_step) | Elapsed: $(elapsed_str) | Spent: $(credit_used)" "urgent" "rotating_light"
        exit 1
    fi
}
...
if not log.exists():
    ...
    for line in log.read_text().splitlines():
        ...
        e = json.loads(line)
        if "opt3_full" not in e.get("label", ""): continue
        d = e.get("domains", {})
...
log "=============================="
log "Full FT — 4-domain cross-domain eval matrix"
log "Steps: $STEPS | Eval samples: $N_EVAL | ntfy: $NTFY_TOPIC"
log "=============================="

notify "Full FT chain started" "4-domain cross-eval | Topic: $NTFY_TOPIC" "default" "rocket"
...
# ─────────────────────────────────────────────
log "=============================="
log "Full FT complete. Elapsed: $(elapsed_str) | Total spend: $(credit_used)"
log "=============================="

notify "Full FT COMPLETE" \
